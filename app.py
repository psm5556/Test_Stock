...
class StockAnalyzer:
    ...
    def _get_extended_period_for_ma(self, period):
        ...
        extended_periods = {
            ...
            '6mo': 'max',    # 6개월 표시 → 2년 데이터 가져오기
            '1y': 'max',     # 1년 표시 → 3년 데이터 가져오기
            '2y': 'max',     # 2년 표시 → 5년 데이터 가져오기
            '5y': 'max',     # 5년 표시 → 최대 데이터 가져오기
            '10y': 'max'     # 5년 표시 → 최대 데이터 가져오기
        }
...
def main():
    ...
    market = st.sidebar.selectbox(
        ...
        format_func=lambda x: {
            ...
            'KOSPI': 'KOSPI (200개)',
            'KOSDAQ': 'KOSDAQ (50개)',
            'FUTURE_LEADERS': '🌟 미래 대장주 엄선 (72개)',
            'AEROSPACE': '🚀 우주항공 섹터 (32개)',
            'QUANTUM': '⚛️ 양자컴퓨터 섹터 (26개)',
            'LONGEVITY': '🧬 노화역전/장수 섹터 (34개)',
            'SYNTHETIC_BIO': '🔬 합성생물학 섹터  (27개)',
            'STABLECOIN': '💰 스테이블코인/암호화폐 섹터 (26개)',
            'DATACENTER_SEMI': '❄️ 데이터센터 반도기술 섹터 (42개)',
            'BCI': '🧠 뇌-컴퓨터 인터페이스(BCI) 섹터 (31개)',
            'DATA_CENTER_ENERGY': '🔋 데이터센터 에너지 공급(SMR,수소,전력공급), 에너지저장시스템(ESS) 섹터 (24개)',
            'DATA_CENTER_INFRASTRUCTURE': '🏗️ 데이터센터 인프라 섹터 (21개)',
            'MEGA_CAP_LEADERS': '🏆 초우량 글로벌 빅테크 섹터 (6개)',
            'CYBERSECURITY': '🛡️ 사이버보안 섹터 (4개)',
            'SATELLITE_COMMUNICATIONS': '🛰️ 위성통신 섹터 (2개)',
            'SUBSEA_CABLES': '🌊 해저케이블 섹터 (3개)',
            'OCEAN_PLASTICS': '♻️ 해양플라스틱 섹터 (1개)'
        }[x]
    )
